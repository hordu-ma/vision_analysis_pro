...
本计划面向“工程基础设施图像识别智能运维系统（无人机巡检 + YOLO 检测）”，以 12 周交付可试点的端到端方案为目标：从数据与训练、推理与 API、边缘 Agent，到生产化与稳态运营能力。
...
- **后端测试**：当前轻量环境基线为 218 passed, 44 skipped（缺少 legacy `runs/train/exp/weights/best.pt`、`models/best.onnx`、`data/images/*` 或可选本地模型产物时跳过对应测试），ruff 全绿
- **前端测试**：90 passed（vitest），ESLint 全绿，生产构建与 3 条 browser E2E 通过；2026-04-22 已完成产品化视觉提升并通过桌面/移动截图复核
- **下一步**：在真实环境样本暂不可得前，优先把系统封装为可部署、可演示、可采集真实样本的输电塔巡检试点系统。多类塔材模型继续保留为实验模型；现场数据进入系统后，再进行样本扩充、人工复核标注、`prototype_v0_2` 训练、评估、ONNX 导出和部署模型替换。

**试点封装路线（2026-05-07）**

1. 系统试点封装成型：部署、启动、演示、metrics、报告、导出和回滚流程可重复。
2. 先接入当前可用模型能力：`stub` 做稳定链路验证，Stage A ONNX 做真实模型路径演示，多类塔材模型暂不作为默认部署模型。
3. 把系统做成真实数据采集入口：Edge Agent、批次 metadata、复核状态和导出链路先准备好。
4. 内置人工复核与标注准备流程：inbox、annotation queue、reviewed label 校验和数据集生成脚本保持可用。
5. 用现有样本做完整试点预演：验证系统链路，不宣称多类模型准确率。
6. 现场测试后再扩样本和训练：训练 `prototype_v0_2`，评估、导出 ONNX、替换试点模型并回归验证。
...
- 当前状态：图片整理、AI 辅助 bbox 标注、YOLO 数据集生成、smoke training 和本地推理 smoke 已完成；当前权重能加载但在正常阈值下没有稳定检测。
- 下一步：先完成试点系统封装和现场数据采集入口；真实数据回流后再改进数据和标注，训练 `prototype_v0_2`。当前暂不导出多类 ONNX，也不作为默认 API/前端模型。
- 详细说明：`docs/multiclass-tower-defect-prototype.md`。

**路线决策（2026-04-19）**

- 近期最佳实践不是串联 DeepLab、YOLO、Transformer、LLM 四段复杂链路，而是先收敛为“裂缝检测试点 + 目标检测工程闭环”。
- `stub` 用于稳定验证前后端与边缘上报链路；Stage A 自训练 YOLO/ONNX 用于真实模型路径；五类 YOLO/ONNX 必须等数据集、权重与评估报告补齐后再作为正式能力启用。
- OpenCV 关键帧抽取进入数据层主线；公开 crack-only 数据集已完成本地 YOLO 转换和 1-epoch smoke training；Stage B 明确为自有视频/图片抽帧、标注、训练和对比闭环；LLM 报告已限制在报告解释层；语义分割和趋势 Transformer 仍作为证据门禁后的后续能力，不能阻塞当前试点闭环。
...
- 可运行的推理引擎（Python 基线 + ONNX Runtime 推理后端）。
...
- ✅ API 与测试：`/api/v1/health`、`/api/v1/inference/image`、`/api/v1/report`、`/api/v1/report/{batch_id}/summary` 闭环，当前轻量后端基线为 218 passed, 44 skipped
...
- 数据与训练：Stage A baseline 训练、评估报告、ONNX 导出与推理烟测已完成；Stage B 数据 intake 工具已完成，现有本地 `data/stage_b_pilot_crack` 是 Stage A 测试集自动标注代理数据，不是真实试点数据；公开代理数据入口（SDNET2018 + RDD2022）和 `data/pilot_inbox/` 前置校验已补齐，真实试点版模型对比仍等待 reviewed pilot labels
- 生产化：CI/Docker/API CLI/最小 metrics/上报持久化与幂等已落地，浏览器级 E2E、上报稳态测试、部署编排与前端产品化视觉提升已完成，告警示例仍可继续细化
- 可选优化：真实样本到位前继续做 public surrogate 验证、试点链路演练、告警阈值调优；MQTT 上报器仅在现场需要时再评估
...
- ✅ 数据与训练：
  ...
  - ✅ `scripts/train.py` 可复现训练；当前 Stage A 权重为 `runs/stage_a_crack/baseline_v0_1/weights/best.pt`
  - ✅ 导出 ONNX：`scripts/export_onnx.py`，当前 Stage A ONNX 为 `models/stage_a_crack/best.onnx`
- ✅ 推理引擎（Python 基线）：
...
- ✅ 工程化基础：
  ...
  - [x] 云端上报批次 SQLite 持久化、`batch_id` 幂等与 API Key 校验

**验收结果**：Agent 功能 ✅ 完成，工程化基础 ✅ 完成，稳态运营能力 🚧 继续迭代

### M4（第 9-12 周）：稳态运营与试点交付 🚧 进行中

**目标**：围绕当前 Stage A YOLO/ONNX 部署模型，完善试点运行、回滚、观测和真实样本接入能力。

**交付物**：

- [ ] 真实样本到位前的 public surrogate 非真实试点验证记录
- [x] 真实试点图片/视频、预标注、人工复核和 Stage B 重训交接清单前置校验
- [ ] 运维能力：灰度发布、模型回滚、集中日志/告警策略（最小可用版）

**验收标准**：

- 真实样本到位后可直接进入 HE-007 真实试点版；没有真实样本时，Stage A 部署、stub 回滚、观测与导出链路可演练
...
- `models/stage_a_crack/best.onnx` 可在本机完成一次推理 smoke test（无论 CPU/GPU）。
...
### Week 9：真实样本接入准备与试点演练

**目标**：在人工复核样本到位前，把数据接收、预标注、复核、上报和回滚流程演练到可交接状态。

**任务**：

- 数据交接：明确真实图片/视频目录、命名规则、设备/线路/时间元数据和复核人字段。
- 预标注演练：使用 Stage A ONNX 对可用样例或 public surrogate 做自动标注流程演练。
- 试点链路演练：重复验证 API、前端、Edge Agent 上报、离线缓存、复核、导出和 stub 回滚。

**DoD / 验收**：

- 输出一份“样本到位后 1 天内可重跑 HE-007”的操作清单和演练记录。

### Week 10：Stage B 真实试点版执行窗口

**目标**：一旦 reviewed positive pilot crack labels 到位，立刻完成 Stage B 训练、评估和部署模型决策。

**任务**：

- 构建数据集：用 `scripts/prepare_stage_b_pilot_dataset.py` 生成并校验 `data/stage_b_pilot_crack/`。
- 训练对比：优先从 Stage A 最佳权重微调，并与 Stage A 在同一 held-out pilot validation set 上比较。
- 决策同步：更新模型推荐、部署文档和 `docs/stage-b-model-comparison.md`。

**DoD / 验收**：

- 在同一试点验证集上输出 Stage A / Stage B 指标、推荐部署模型和回滚路径。
...
5. **真实样本交接策略**：图片/视频、元数据、预标注、人工复核和模型回滚如何衔接（Week 9 定）
