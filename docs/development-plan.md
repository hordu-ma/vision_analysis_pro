...
- API 与测试：已有基础 API 入口与测试目录（具体进度以 `docs/progress.md` 为准）。

### 4.2 主要差距
- 数据与训练：缺少 `data.yaml`、训练脚本、可复现的数据切分与评估流程。
- 推理：需要将真实 YOLO 输出稳定映射到项目的 schema（并补齐异常路径/性能策略）。
...
- 数据与训练：
   ...
   - `scripts/train.py`（或等价命令文档）可复现训练；产出 `models/best.pt`。
   - `scripts/export.py` 导出 ONNX（产出 `models/best.onnx`）。
- 推理引擎（Python 基线）：
