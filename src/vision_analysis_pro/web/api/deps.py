...
@lru_cache(maxsize=1)
def _load_hf_crack_engine(model_path: str) -> HFCrackInferenceEngine:
    """加载 Hugging Face 裂缝检测引擎（带缓存）。"""
    path = Path(model_path)
    if not path.exists():
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail=f"HF 模型目录不存在: {model_path}",
        )
    try:
        return HFCrackInferenceEngine(path)
    except ImportError as exc:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail=f"未安装 HF 裂缝模型依赖，无法加载模型: {exc}",
        ) from exc
    except Exception as exc:  # pragma: no cover
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail=f"HF 模型加载失败: {exc}",
        ) from exc
...
def clear_inference_engine_caches() -> None:
    ...
    _load_hf_crack_engine.cache_clear()
...
def get_inference_engine(
    ...
    - "hf_crack": 使用 HFCrackInferenceEngine (公开裂缝检测参考模型)
    ...
    - HF_CRACK_MODEL_PATH: HF 裂缝模型目录，默认 "models/only-crack-I"
    ...
    if engine_type == "hf_crack":
        model_path = os.getenv("HF_CRACK_MODEL_PATH", "models/only-crack-I")
        return _load_hf_crack_engine(model_path)
