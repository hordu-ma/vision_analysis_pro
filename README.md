...
- **默认运行口径**：`stub` 仅用于内部链路自检；客户正式演示的检测结果优先使用 Stage A ONNX，必要时回退 Stage A YOLO；当前多类塔材模型保留为实验模型，不作为默认部署模型。
...
- **训练路径**：`scripts/prepare_multiclass_tower_dataset.py` 会从 `data/multiclass_inbox/reviewed_labels/` 生成 `data/multiclass_tower_defect/`；当前已完成 1-epoch YOLO smoke training，证明原型训练链路可运行，但不宣称准确率。
- **推理结论**：`scripts/smoke_multiclass_tower_inference.py` 已验证本地权重可以加载，但当前模型在正常阈值下没有稳定检测；下一步先改进数据和标注，不急于导出 ONNX 或接入前端。
...
├── models/                     # 训练/导出模型产物
├── web/                        # 前端（Vue3 + Vite + TS）
├── tests/                      # Python 测试（当前轻量基线 218 passed, 44 skipped）
├── docs/                       # 计划与进度文档
...
- 后端：`uv run pytest`（当前本地轻量环境为 218 passed, 44 skipped；legacy `models/best.onnx`、`data/images/*` 或可选本地模型产物缺失时会跳过对应测试）
